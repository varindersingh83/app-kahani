...
const IMAGE_TEXT_NEGATIVE_PROMPT =
  "No scary imagery, no harsh lighting, no inconsistent character traits. No text, letters, words, labels, headings, captions, speech bubbles, signs, logos, watermarks, page numbers, panel numbers, or camera/composition words rendered anywhere in the image.";
...
export function runCharacterConsistencyAgent(request: StoryRequest) {
  ...
  return {
    ...
    negativePrompt: IMAGE_TEXT_NEGATIVE_PROMPT,
  };
}
...
function buildMasterSheetPrompt(
  ...
  return [
    ...
    ...storyPlan.pages.map(
      (page) =>
        `Scene ${page.pageNumber}, row ${page.sheetPlacement.row}, column ${page.sheetPlacement.col}: ${page.beat} | Visual focus: ${page.visualFocus} | Emotion: ${page.emotion}`,
    ),
    `IMPORTANT IMAGE RULE: the illustration itself must contain artwork only. Do not paint or draw any readable text, labels, headings, camera direction words, page numbers, panel numbers, captions, speech bubbles, signs, logos, or watermarks.`,
    ...
    `return the canonical 12-page story text as separate structured JSON in the text response only, never as text inside the image`,
    `Negative prompt: ${IMAGE_TEXT_NEGATIVE_PROMPT}`,
  ]
    .filter(Boolean)
    .join("\n");
}
...
function buildStoryPages(childName: string, topic: string, pageSlots: BookSetup["pageSlots"]) {
  ...
  return pageSlots
    ...
    .map((slot, index): StoryPlanPage => {
      const beat = `${childName} ${beats[index]}.`;
      const visualFocus = `watercolor scene of ${childName}, ${cleanedTopic}, consistent outfit, warm emotional storytelling, artwork only with no written words`;
      const emotion = [
        "curious",
        "uncertain",
        "frustrated",
