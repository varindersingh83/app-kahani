...
export function buildSheetPlan(input: {
  ...
  const panelLines = tiles.map(
    (tile) =>
      `Scene ${tile.pageNumber}, row ${tile.row}, column ${tile.col}: ${tile.beat} | Visual focus: ${tile.visualFocus} | Emotion: ${tile.emotion}`,
  );
...
function buildPanelBeat(brief: BookBrief, pageNumber: number) {
  ...
  return [
    ...
    `A near portrait view shows the challenge, worry, or lonely moment that starts the story.`,
...
function buildPanelVisualFocus(brief: BookBrief, pageNumber: number) {
  ...
  const focus = [
    ...
    `${sharedStyle}, near portrait view of the challenge`,
    `${sharedStyle}, first action or attempt`,
    `${sharedStyle}, searching or moving through the world`,
    `${sharedStyle}, listening or asking for help`,
  ...
  return focus[pageNumber - 1] ?? sharedStyle;
}
  ...
  return JSON.stringify(
    {
      ...
      pages: storyPlan.pages.map((page) => ({
        ...
        composition: suggestComposition(page.pageNumber),
