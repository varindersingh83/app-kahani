...
test("Phase B cover prompt includes the image brief and character lock", () => {
  ...
  assert.match(prompt, /Artwork only: no text/i);
...
test("Phase B storyboard prompt builds a 3x4 sheet brief with page slots", () => {
  ...
  assert.match(prompt, /Scene 1, row 1, column 1/);
  assert.match(prompt, /Scene 12, row 4, column 3/);
  assert.match(prompt, /artwork only/i);
  assert.doesNotMatch(prompt, /Panel 1:/);
  assert.doesNotMatch(prompt, /close-up/i);
});
...
test("Phase B storyboard sheet plan preserves the deterministic page map", () => {
  ...
  assert.doesNotMatch(plan.sheetPrompt, /Panel 1:/);
  assert.doesNotMatch(plan.sheetPrompt, /close-up/i);
});
...
test("Sheet helper still emits a deterministic 3x4 grid prompt", () => {
  ...
  assert.match(plan.sheetPrompt, /Scene 1, row 1, column 1:/);
  assert.match(plan.sheetPrompt, /Scene 12, row 4, column 3:/);
