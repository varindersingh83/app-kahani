...
test("sheet plan combines the brief into one storyboard-sheet prompt", () => {
  ...
  assert.match(plan.sheetPrompt, /artwork only/i);
});
...
test("sheet prompt follows the master storyboard structure", () => {
  ...
  assert.match(plan.sheetPrompt, /canonical 12-page story text in structured JSON/i);
  assert.match(plan.sheetPrompt, /Scene 1, row 1, column 1:/i);
  ...
  assert.match(plan.sheetPrompt, /Scene 12, row 4, column 3:/i);
